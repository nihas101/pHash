...
(defonce ^:private compr-path-prefix
  "test/de/nihas101/phash/test_images/compr/")

(defonce ^:private blur-path-prefix
  "test/de/nihas101/phash/test_images/blur/")

(defonce ^:private misc-path-prefix
  "test/de/nihas101/phash/test_images/misc/")

(defonce ^:private rotd-path-prefix
  "test/de/nihas101/phash/test_images/rotd/")

(defonce ^:private file-names
  ["architecture_2"
   "architecture1"
   "bamarket115"
   "butterflywallpaper"
...
(defonce compr (mapv #(u/load-image (str compr-path-prefix % ".jpg"))
                     file-names))

(defonce blur (mapv #(u/load-image (str blur-path-prefix % ".bmp"))
                    file-names))

(defonce misc (mapv #(u/load-image (str misc-path-prefix % ".bmp"))
                    file-names))

(defonce rotd (mapv #(u/load-image (str rotd-path-prefix % ".bmp"))
                    file-names))